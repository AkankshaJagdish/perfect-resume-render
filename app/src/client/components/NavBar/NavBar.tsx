...
import logo from "../../static/application-approved-icon-2.svg";
          ...
          >
            <div className="flex items-center gap-6">
              ...
              <ul className="ml-4 hidden items-center gap-6 lg:flex">
                {renderNavigationItems(navigationItems, undefined, !!user)}
              </ul>
            </div>
            <NavBarMobileMenu
          ...
          <SheetHeader>
            <SheetTitle className="flex items-center">
              <WaspRouterLink to={routes.LandingPageRoute.to}>
                <span className="sr-only">PerfectResume</span>
...
function renderNavigationItems(
  ...
  return navigationItems.map((item) => {
    return (
      <li key={item.name}>
        <ReactRouterLink
          to={getNavigationItemTarget(item, isAuthenticated)}
          className={menuStyles}
          onClick={setMobileMenuOpen && (() => setMobileMenuOpen(false))}
          target={
            getNavigationItemTarget(item, isAuthenticated).startsWith("http")
              ? "_blank"
              : undefined
          }
        >
...
function getNavigationItemTarget(
  item: NavigationItem,
  isAuthenticated: boolean,
): string {
  if (item.name === "Resume" && !isAuthenticated) {
    return routes.LoginRoute.to;
  }
